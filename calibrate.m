...
ListAllMolecules=uicontrol(SelectionPanel,'Style','ListBox',...
    ...
    'KeyPressFcn', @handlekeyboardevents,...
...
b_left = uicontrol(SelectionPanel,'style','pushbutton',...
          'string','<-',...
...
b_autozoom = uicontrol(PreviewPanel,'style','pushbutton',...
          'string','Autozoom',...
...
b_refresh = uicontrol(PreviewPanel,'style','pushbutton',...
          'string','Refresh',...
          'Callback',@autozoombutton,...
          'Units','normalized',...
          'Enable', 'off',...
...
b_area = uicontrol(PreviewPanel,'style','pushbutton',...
          'string','Area',...
...
b_fitthis = uicontrol(PreviewPanel,'style','pushbutton',...
          'string','Fit this',...
...
b_fitall = uicontrol(PreviewPanel,'style','pushbutton',...
          'string','Fit all',...
          'Callback',@fitbuttonclick,...
          'Units','normalized',...
          'Enable', 'off',...
...
% need to set properties in case the calibration list is not empty
% --> write calibration molecules into litsbox
if ~isempty(handles.calibration.namelist)
    ...
    ranges2listbox(1,1);
% --> enable remove molecules button
    set(b_left, 'Enable', 'on');    
...
set(massmethode,'Value',find(strcmp(get(massmethode,'String'),handles.calibration.massoffsetmethode)));
set(e_massoffsetorder,'String',num2str(handles.calibration.massoffsetparam));
...
set(resolutionmethode,'Value',find(strcmp(get(resolutionmethode,'String'),handles.calibration.resolutionmethode)));
set(e_resolutionorder,'String',num2str(handles.calibration.resolutionparam));

% enable e_resolutionorder and e_massoffsetorder in case polynomial was chosen
string_massmethode = get(massmethode,'String');
string_resolutionmethode = get(resolutionmethode,'String');
if strcmp(string_massmethode{get(massmethode,'Value')},'Polynomial')
    set(e_massoffsetorder,'Enable','on');
end
if strcmp(string_resolutionmethode{get(resolutionmethode,'Value')},'Polynomial')
    set(e_resolutionorder,'Enable','on');
end
...
%################### INTERNAL FUNCTIONS
    ...
    function handlekeyboardevents(source,eventdata)
    ...
    function massmethodechange(hObject, ~)
        ...
        handles.calibration.massoffsetmethode=methode{index};
        % enable edit of massoffset order only for Polynomial
        if strcmp(methode{index},'Polynomial')
            set(e_massoffsetorder,'Enable','on');
        else
            set(e_massoffsetorder,'Enable','off');
        end
        guidata(Parent,handles);
    end
    ...
    function resolutionmethodechange(hObject, ~)
        ...
        methode=get(hObject,'String');
        index=get(hObject,'Value');
        handles.calibration.resolutionmethode=methode{index};
        % enable edit of resolution order only for Polynomial
        if strcmp(methode{index},'Polynomial')
            set(e_resolutionorder,'Enable','on');
        else
            set(e_resolutionorder,'Enable','off');
        end
        guidata(Parent,handles);        
    end
    ...
    function updatepolynomials(hObject, ~)
        ...
        switch(lower(handles.calibration.massoffsetmethode))
            ...
            otherwise
                % used to be spaps case, was removed from list
    ...
    function previewpaneledit(value)
        ...
        set(b_area,'Enable',value);
        set(b_fitthis,'Enable',value);
        set(b_fitall,'Enable',value);
        set(b_refresh,'Enable',value);
        set(b_autozoom,'Enable',value);
    end
...
%       
    ...
    function removefromlist(hObject, ~)
        ...
        % enable removemolecules button if list is empty
        if numel(get(ListRanges,'string'))==0
            set(b_left, 'Enable', 'off');
        end
